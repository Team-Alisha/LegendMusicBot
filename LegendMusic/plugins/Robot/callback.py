...
from LegendMusic.misc import SUDOERS, db
from LegendMusic.utils import bot_sys_stats
from LegendMusic.utils.database import (is_active_chat,
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await LegendX.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if "live_" in queued:
            ...
            try:
                await LegendX.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
